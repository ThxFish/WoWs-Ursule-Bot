...
from .models import ResetPlan
from .planner import BRITISH_LIGHT_CRUISER_LINE, line_progress_xp
from .service import dashboard_context, guarded_sync
...
async def execute_command(command: str, allow_sync: bool) -> str:
    if command == "帮助":
        return help_text()
    if command == "进度":
    ...
    if command == "同步":
        if not allow_sync:
            return "为保护账户，群聊不允许执行同步；请使用已授权好友私聊机器人。"
        return await _sync_text()
    return "命令格式不正确。\n" + help_text()
