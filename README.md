...
应用不保存你的 Wargaming 账号密码。Windows 本地预览时，打开“设置 → 军械库登录状态”，点击“一键打开军械库登录窗口”。已有会话会自动复用；只有会话确实失效时才需要重新登录。应用会自动点击右上角“钱包”（并以钱包页面直达作为后台兜底）来触发按需加载的 `inventory` 请求。页面识别到资源和物品数据后会自动保存状态、关闭窗口并立即同步，无需返回终端按 Enter。

每次后台采集成功后，应用都会安全地回写最新 Cookie 和 IndexedDB 登录状态，以减少反复登录。若网页按钮无法启动浏览器，可使用备用命令：

```powershell
.\scripts\auth-armory.ps1
```

在弹出的 Chromium 中手动登录欧服军械库；资源加载完成后窗口会自动关闭。登录助手会生成：

```text
data/auth/armory-storage.json
```

本机预览会直接使用该文件。无图形桌面的 Linux/Docker 主机无法把登录窗口弹到远程 Windows 浏览器，因此首次部署或会话真正失效时，应在 Windows 登录，再从设置页导入该文件或安全复制到服务器的 `data/auth/`。该文件包含敏感 Cookie 和站点状态，不要提交到 Git、上传到网盘或分享给他人。
...
先在设置页点击“一键打开军械库登录窗口”。当前采集器按 JSON 数据结构识别 inventory、资源余额和加成卡，不再依赖固定的 `account/info` URL；错误详情只记录不含查询参数的接口路径。若登录状态有效，窗口会自动完成，无需再次输入账号。若军械库改版后仍无法识别，旧数据会保留，数据源状态会记录诊断信息。
